<template>
  <div class="space-y-6">
    <div class="flex items-center justify-between">
      <h3 class="text-lg font-semibold text-gray-900 dark:text-slate-100">References</h3>
    ...
    <div class="space-y-6">
      <div v-for="(r, index) in resumeStore.references" :key="r.id"
        ...
        <div class="flex items-start justify-between mb-4">
          <h4 class="font-medium text-gray-900 dark:text-slate-100">Reference {{ index + 1 }}</h4>
          <button @click="remove(r.id)" aria-label="Remove" class="p-1 text-red-400 hover:text-red-600">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            <label class="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-2">Title / Position</label>
          ...
          <div>
            <label class="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-2">Mobile</label>
          ...
          <div>
            <label class="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-2">Relation</label>
    ...
    <div v-if="resumeStore.references.length === 0"
      ...
      <UserGroupIcon class="w-12 h-12 text-gray-400 dark:text-slate-500 mx-auto mb-4" />
      <h3 class="text-lg font-medium text-gray-900 dark:text-slate-100 mb-2">No references added</h3>
      <p class="text-gray-600 dark:text-slate-400 mb-4">Add people who can vouch for your work</p>
