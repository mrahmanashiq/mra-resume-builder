<template>
  <div class="space-y-6">
    <div class="flex items-center justify-between">
      <h3 class="text-lg font-semibold text-gray-900 dark:text-slate-100">Certifications</h3>
    ...
    <div class="space-y-6">
      <div v-for="(certification, index) in resumeStore.certifications" 
        ...
        <!-- Certification Name -->
        <div class="mb-4">
          <label class="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-2">Certification Name</label>
          <input type="text" 
        ...
        <!-- Issuer -->
        <div class="mb-4">
          <label class="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-2">Issuing Organization</label>
          <input type="text" 
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div>
            <label class="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-2">Expiry Date (Optional)</label>
            <BaseDatePicker mode="month"
                            :model-value="certification.expiryDate"
                            @update:model-value="updateCertification(certification.id, 'expiryDate', $event)" />
          </div>
          <div>
            <label class="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-2">Credential ID</label>
            <input type="text" 
    ...
    <div v-if="resumeStore.certifications.length === 0" 
      ...
      <TrophyIcon class="w-12 h-12 text-gray-400 dark:text-slate-500 mx-auto mb-4" />
      <h3 class="text-lg font-medium text-gray-900 dark:text-slate-100 mb-2">No certifications added</h3>
      <p class="text-gray-600 dark:text-slate-400 mb-4">Add your professional certifications and credentials</p>
      <button @click="addCertification" class="btn-primary">
